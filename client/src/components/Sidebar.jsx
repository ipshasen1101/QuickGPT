...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  return (
    <div className = {`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from -[#242124]/30 to -[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
        ...
        <button className='flex w-full justify-center items-center  py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer '>
     ...
     <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'> 
        {//flex-1: Allows a flex item to grow and shrink as needed, taking up all available remaining space in its parent flex container,overflow-y-scroll: Forces a vertical scrollbar to appear. This ensures that if the content inside the element is taller than the element itself, the user can scroll through it,space-y-3: Adds a vertical gap between child elements. 
    ...
    <div  className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        <p className ='flex-1 flex-sm dark:text-primary truncate'>{user? user.name:  'Login your account'}</p>
...
export default Sidebar //The div is just a box—it doesn't have a "checked" state. Only the <input> has that state. In CSS (and Tailwind), you need a way to say: "When that other element (the input) is checked, change my color". 

// checked:bg-purple-600 (Wrong): This tells the div to check its own state. Since a div can't be "checked," nothing happens.
// peer-checked:bg-purple-600 (Correct): This tells the div to look at its peer (the sibling element marked with the peer class). When that peer becomes checked, the div applies the purple background